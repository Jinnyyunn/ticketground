...
- [PORTS.md](PORTS.md): 브랜치별 포트 배정표 (관리자/사용자 포트 포함)
...
> 중요: 어떤 페이지든 수정/업데이트 작업을 시작하기 전에 [PORTS.md](PORTS.md)를 먼저 읽는다. 브랜치별로 어떤 포트가 관리자 콘솔용이고 어떤 포트가 사용자 페이지용인지 정리되어 있으며, 새 브랜치나 새 포트가 생기면 그 즉시 이 파일을 갱신한다.
