...
export function CancelHistoryPanel() {
  ...
  return (
    <section id="cancel-history" className="mt-8 scroll-mt-[176px]" aria-live="polite">
      ...
      <h2 className="mt-2 text-[30px] font-bold text-ink-2">취소 내역</h2>
      {history.length === 0 ? (
          ...
          {history.map((entry) => (
            <article key={entry.reservationId} className="rounded-[10px] border border-[#eee] p-5" data-cancel-history-row>
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded bg-[#fff3d0] px-2 py-1 text-sm font-bold text-ticketground">취소요청</span>
                <span className="text-sm font-bold text-[#777]">사유 · {entry.reason}</span>
              </div>
              <h3 className="balanced-title mt-3 text-[22px] font-bold text-ink-2">{entry.showTitle}</h3>
              <p className="mt-2 break-words text-base text-[#666]">{entry.venue}</p>
              <p className="mt-1 text-base font-bold">
                {entry.date} {entry.time} · {entry.seat}
              </p>
              <p className="mt-1 text-sm text-[#777]">예매번호 {entry.reservationId}</p>
