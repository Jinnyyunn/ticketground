  ...
  return (
    <TicketgroundSurface className="overflow-hidden p-0 shadow-ticket-3">
      <div className="grid gap-1 border-b border-line bg-surface px-5 py-4">
        ...
        <p className="text-sm font-bold text-ink-3">등급, 구역, 가격, 연석 조건만 고르고 좌석은 추첨 후 공개됩니다.</p>
      </div>
      <div className="grid gap-5 p-5">
        ...
        <p className="rounded-lg border border-line bg-background p-3 text-sm font-bold text-ink-3" data-testid="draw-guard">좌석번호는 추첨 완료 전까지 공개되지 않으며 직접 선택할 수 없습니다.</p>
        <div className="grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={onDraw} disabled={drawing || apiBusy || filteredCandidates.length === 0} className="h-12 rounded-sm bg-ticketground px-5 text-base font-black text-white shadow-ticket-2 transition-colors active:translate-y-px disabled:bg-ink-4" data-testid="resale-draw">
            {drawing ? "후보 순환 중" : `조건부 랜덤 매칭 시작 · 후보 ${filteredCandidates.length}건`}
          </button>
          <button type="button" onClick={onBackendPurchase} disabled={apiBusy} className="h-12 rounded-sm bg-ink px-5 text-base font-black text-on-ink shadow-ticket-2 transition-colors hover:bg-ink-2 active:translate-y-px disabled:bg-ink-4" data-testid="resale-purchase">
            즉시 구매 매칭
          </button>
        </div>
        <div
...
function BackendMatchResult({ backendResult, reservationId }: { readonly backendResult: ApiResaleResult; readonly reservationId: string }) {
  return (
    <div className="grid gap-3">
      ...
      <Link className="inline-flex h-11 items-center justify-center rounded-sm bg-ink px-5 text-sm font-black text-on-ink shadow-ticket-2 transition-colors hover:bg-ink-2 active:translate-y-px" href={`/reservation/${reservationId}?ticketId=${backendResult.ticket.id}`} data-testid="confirm-cta">
        매칭 좌석으로 확인하기
      </Link>
    </div>
  );
...
function LocalMatchResult({ reservationId, result, resultFee }: { readonly reservationId: string; readonly result: MatchCandidate; readonly resultFee: number }) {
  return (
    <div className="grid gap-3">
      ...
      <Link className="inline-flex h-11 items-center justify-center rounded-sm bg-ink px-5 text-sm font-black text-on-ink shadow-ticket-2 transition-colors hover:bg-ink-2 active:translate-y-px" href={`/reservation/${reservationId}`} data-testid="confirm-cta">
        매칭 좌석으로 확인하기
      </Link>
    </div>
  );
