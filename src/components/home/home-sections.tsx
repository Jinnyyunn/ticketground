...
type SectionDictProps = {
  readonly dict: Dictionary;
};

export function HomeHeroSection({ dict }: SectionDictProps) {
  return (
    <section data-section="spec-hero" className="ticketground-container min-w-0 pt-8">
      <MiniShowCarousel
        shows={[featuredShow, ...miniShows]}
        ariaSlides={dict.home.hero.ariaSlides}
        ariaPrev={dict.home.hero.ariaPrev}
        ariaNext={dict.home.hero.ariaNext}
        primaryCtaLabel={dict.home.hero.ctaBook}
        secondaryCtaLabel={dict.home.hero.ctaNotify}
      />
    </section>
  );
}
...
export function RealtimeTop10Section({ items, dict }: SectionDictProps & { readonly items: readonly RankingShow[] }) {
  return (
    <section data-section="realtime-top10" className="ticketground-container mt-16">
      <SectionHead title={dict.home.realtimeTop10.title} subtitle={dict.home.realtimeTop10.subtitle} moreHref="/contents/ranking" moreLabel={dict.home.sectionMore} />
      <div className="lg:hidden">
        <CarouselRow className="mt-1 pb-1" prevLabel={dict.common.carouselPrev} nextLabel={dict.common.carouselNext}>
          {items.map((item) => (
...
function TicketOpenCard({ item, alertLabel, locale }: { readonly item: (typeof ticketOpens)[number]; readonly alertLabel: string; readonly locale: Locale }) {
  return (
    <Link
      href="/open"
    ...
    >
      ...
      <div className="min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className="text-xs font-black text-ticketground">{formatMonthDay(locale, item.month, item.day)}</p>
        ...
        <TicketOpenAlertAction label={alertLabel} />
      </div>
    </Link>
  );
}

export function TicketOpenSection({ dict, locale }: SectionDictProps & { readonly locale: Locale }) {
  return (
    <section data-section="ticket-open" className="ticketground-container mt-16">
      <SectionHead title={dict.home.ticketOpen.title} subtitle={dict.home.ticketOpen.subtitle} moreHref="/open" moreLabel={dict.home.sectionMore} />
      <div className="lg:hidden">
        <CarouselRow className="mt-1 pb-1" prevLabel={dict.common.carouselPrev} nextLabel={dict.common.carouselNext}>
          {ticketOpens.map((item) => (
            <div key={`${item.month}.${item.day}-${item.title}`} className="w-[264px] shrink-0">
              <TicketOpenCard item={item} alertLabel={dict.home.ticketOpen.alertButton} locale={locale} />
            </div>
          ))}
        </CarouselRow>
      </div>
      <div className="hidden lg:mt-1 lg:grid lg:grid-cols-4 lg:gap-4">
        {ticketOpens.map((item) => (
          <TicketOpenCard key={`${item.month}.${item.day}-${item.title}`} item={item} alertLabel={dict.home.ticketOpen.alertButton} locale={locale} />
        ))}
      </div>
    </section>
  );
...
export function OfficialResaleSection({ dict }: SectionDictProps) {
  const { officialResale } = dict.home;
  const steps = [
    { icon: TicketCheck, ...officialResale.steps.verifyTicket },
    { icon: Workflow, ...officialResale.steps.autoPolicyCheck },
    { icon: QrCode, ...officialResale.steps.qrProtection },
  ];
  return (
    <section data-section="official-resale" className="ticketground-container mt-16">
      <SectionHead
        title={officialResale.title}
        subtitle={officialResale.subtitle}
        moreHref="/resale"
        moreLabel={dict.home.sectionMore}
        badge={officialResale.badge}
      />
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
        ...
        >
          ...
          <p className="flex items-center gap-1.5 text-sm font-black text-on-ink/60">
            ...
            {officialResale.poolLabel}
          </p>
          <h3 className="balanced-title mt-3 text-[24px] font-black leading-tight sm:text-[30px]">{officialResale.cardTitle}</h3>
          <p className="mt-3 max-w-[560px] break-keep text-sm leading-relaxed text-on-ink/75 sm:text-base">
            {officialResale.cardDescription}
          </p>
...
function EditorialCard({ event, index, title, eyebrow, cta }: {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex h-full min-h-[166px] flex-col">
        ...
        <h3 className="balanced-title mt-7 text-3xl font-black leading-tight sm:text-4xl">{title}</h3>
        <span className={cn("mt-8 inline-flex h-9 w-fit items-center rounded-lg border px-3 text-sm font-black transition-colors", tone.cta)}>
          {cta} <span className="ml-1" aria-hidden="true">→</span>
        </span>
      </div>
    </Link>
  );
...
export function EditorialEventsSection({ dict }: SectionDictProps) {
  const { editorialEvents } = dict.home;
  return (
    <section data-section="editorial-events" className="ticketground-container mt-16">
      <SectionHead title={editorialEvents.title} subtitle={editorialEvents.subtitle} moreHref="/event/ticketground-day" moreLabel={dict.home.sectionMore} />
      <div className="lg:hidden">
        <CarouselRow className="pb-1" prevLabel={dict.common.carouselPrev} nextLabel={dict.common.carouselNext}>
          {events.map((event, index) => (
            <div key={event.id} className="w-[240px] shrink-0 sm:w-[260px]">
              <EditorialCard
                event={event}
                index={index}
                title={editorialEvents.items[event.id].title}
                eyebrow={editorialEvents.eyebrow}
                cta={editorialEvents.cta}
              />
            </div>
          ))}
        </CarouselRow>
...
export function GenreRecommendationsSection({ dict }: SectionDictProps) {
  return (
    <section data-section="genre-recommendations" className="ticketground-container mt-16">
      <SectionHead title={dict.home.genreRecommendations.title} subtitle={dict.home.genreRecommendations.subtitle} moreHref="/contents/genre" moreLabel={dict.home.sectionMore} />
      <GenreRecommendationsTabs
        groups={genreRecommendations}
        categoryLabels={dict.header.categories}
        ariaTabs={dict.home.genreRecommendations.ariaTabs}
        emptySuffix={dict.home.genreRecommendations.emptySuffix}
        carouselPrevLabel={dict.common.carouselPrev}
        carouselNextLabel={dict.common.carouselNext}
      />
    </section>
  );
}
...
function ShortcutCard({ shortcut, label, helper }: { readonly shortcut: (typeof shortcuts)[number]; readonly label: string; readonly helper: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        <strong className="clamp-1 block text-sm font-black text-ink">{label}</strong>
        <span className="clamp-1 mt-0.5 block text-xs text-ink-3">{helper}</span>
      </span>
    </Link>
  );
}

export function ShortcutsSection({ dict }: SectionDictProps) {
  const { shortcuts: shortcutsDict } = dict.home;
  return (
    <section data-section="shortcuts" className="ticketground-container my-16">
      <SectionHead title={shortcutsDict.title} moreHref="/contents/shortcuts" moreLabel={dict.home.sectionMore} />
      <div className="lg:hidden">
        <CarouselRow className="pb-1" prevLabel={dict.common.carouselPrev} nextLabel={dict.common.carouselNext}>
          {shortcuts.map((shortcut) => (
            <div key={shortcut.id} className="w-[100px] shrink-0">
              <ShortcutCard shortcut={shortcut} label={shortcutsDict.items[shortcut.id].label} helper={shortcutsDict.items[shortcut.id].helper} />
            </div>
          ))}
        </CarouselRow>
      </div>
      <div className="hidden lg:grid lg:grid-cols-6 lg:gap-3">
        {shortcuts.map((shortcut) => (
          <ShortcutCard key={shortcut.id} shortcut={shortcut} label={shortcutsDict.items[shortcut.id].label} helper={shortcutsDict.items[shortcut.id].helper} />
        ))}
      </div>
    </section>
  );
...
export function GroupBookingBanner({ dict }: SectionDictProps) {
  const { groupBooking } = dict.home;
  return (
    <section data-section="group-booking" className="ticketground-container mt-16">
      ...
      >
        <span className="flex min-w-0 items-center gap-3">
          ...
          <span className="min-w-0">
            <strong className="block text-sm font-black text-ink sm:text-base">{groupBooking.title}</strong>
            <span className="mt-0.5 block truncate text-xs font-bold text-ink-3 sm:text-sm">{groupBooking.description}</span>
          </span>
        </span>
