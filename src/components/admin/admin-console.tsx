...
import { Field, visibleUnownedTickets, WorkspacePanel, valueFromForm } from "./console-primitives";
...
export function AdminConsolePage({ workspace = "overview" }: { readonly workspace?: WorkspaceKey }) {
  ...
  const refreshWorkspace = useCallback(async (activeSession: AdminSession): Promise<void> => {
    ...
    setData(await apiRequest<WorkspaceData>(`/api/admin/workspaces/${workspace}`));
  }, [workspace]);
  ...
  const visibleTickets = useMemo(() => visibleUnownedTickets(data), [data]);
