...
export function renderCanvasObject(object: ChartObject, selectedSeatIds: readonly string[], showLabels: boolean): ReactNode {
  ...
  if (object.type === "rectangle") return <RectangleNode object={object} />;
  if (object.type === "booth") return <g transform={object.rotation ? `rotate(${object.rotation} ${object.x + object.width / 2} ${object.y + object.height / 2})` : undefined}><rect x={object.x} y={object.y} width={object.width} height={object.height} rx="2" fill="var(--editor-booth)" stroke="var(--editor-seat-stroke)" /><text x={object.x + object.width / 2} y={object.y + object.height / 2 + 4} textAnchor="middle" fontSize="11">부스</text></g>;
  ...
  if (object.type === "text") return <text x={object.position.x} y={object.position.y} textAnchor={object.align === "left" ? "start" : object.align === "right" ? "end" : "middle"} fontSize={object.fontSize ?? 18} fontWeight={object.weight ?? 500} fill={object.color ?? "var(--editor-text)"} transform={object.rotation ? `rotate(${object.rotation} ${object.position.x} ${object.position.y})` : undefined}>{object.text}</text>;
  if (object.type === "image") return <image href={object.href} x={object.x} y={object.y} width={object.width} height={object.height} opacity={object.opacity ?? 1} transform={object.rotation ? `rotate(${object.rotation} ${object.x + object.width / 2} ${object.y + object.height / 2})` : undefined} preserveAspectRatio="xMidYMid meet" />;
