...
export function ReferenceStart({ onBlank, onReady }: ReferenceStartProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[var(--editor-panel)] p-6" data-testid="seat-designer-v2-reference-start">
      <section className="w-full max-w-[680px] rounded border border-[var(--editor-border)] bg-white p-8 shadow-[var(--editor-elevation)]">
        ...
        <label className="mb-4 block text-sm font-medium text-[var(--editor-text)]" htmlFor="v2-venue"><span className="mb-2 flex items-center gap-2"><Building2 className="size-4" />적용 공연장</span><select id="v2-venue" data-testid="seat-designer-v2-venue" value={venueId} onChange={(event) => setVenueId(event.currentTarget.value)} className="h-10 w-full rounded border border-[var(--editor-border)] bg-white px-3 outline-none focus:border-[var(--editor-accent)]"><option value="">공연장을 선택하세요</option>{venues.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
