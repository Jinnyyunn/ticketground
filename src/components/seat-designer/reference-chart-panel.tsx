  ...
  return (
    <section className="space-y-4">
      <label className="flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#b8c6d5] bg-[#f8fafc] px-5 text-center hover:border-[#0784fa] hover:bg-[#f3f8ff]">
        ...
        <span className="text-sm font-semibold">{sourceFile?.name ?? "JPG, PNG, GIF, WEBP, SVG 또는 PDF 도면 선택"}</span>
        <span className="mt-1 text-xs text-[#667085]">최대 10MB · 원본 메타데이터 제거 · PDF 페이지 선택 지원</span>
        <input className="sr-only" type="file" accept="image/png,image/jpeg,image/gif,image/webp,image/svg+xml,application/pdf" onChange={(event) => {
      ...
      <div className="flex justify-end gap-2">
        {onBack && <button type="button" disabled={busy} className="mr-auto rounded-md border border-black/10 px-4 py-2 text-sm disabled:opacity-40" onClick={onBack}>이전</button>}
