...
class TossCheckoutTest {
  ...
  fun `unconfigured Toss fails closed before checkout is actionable`() = runTest {
    ...
    assertEquals(CheckoutError.ProviderUnavailable, assertFails { coordinator.prepare("ticket-1", TossPaymentMethod.CREDIT_CARD, "stable-1") })
  ...
  fun `successful confirmation is sent once clears payment key and refetches authoritative ticket`() = runTest {
    ...
    val request = coordinator.prepare("ticket-1", TossPaymentMethod.CREDIT_CARD, "stable-1")
  ...
  fun `retry continuity reuses only the non secret stable key after coordinator recreation`() = runTest {
    ...
    assertEquals(
      "stable-original",
      first.prepare("ticket-1", TossPaymentMethod.CREDIT_CARD, "stable-original").idempotencyKey,
    )

    val recreated = TossCheckoutCoordinator(gateway, PersistentCheckoutRetryStore(persistence))
    val retried = recreated.prepare("ticket-1", TossPaymentMethod.CREDIT_CARD, "new-unstable-key")
  ...
  @Test
  fun `unknown or already owned ticket state cannot start another confirmation`() = runTest {
    val gateway = FakeCheckoutGateway(configured = true, initialStatus = "OWNED")
    val coordinator = TossCheckoutCoordinator(gateway, InMemoryCheckoutRetryStore())

    assertEquals(
      CheckoutError.TicketUnavailable,
      assertFails { coordinator.prepare("ticket-1", TossPaymentMethod.CREDIT_CARD, "stable-1") },
    )
    assertEquals(0, gateway.confirmCalls)
  }
...
private class FakeCheckoutGateway(
  ...
  initialStatus: String = "PENDING_PAYMENT",
) : TossCheckoutGateway {
