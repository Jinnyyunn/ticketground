...
| Android tablet, real installed app | Home -> tap Search -> tap event -> Event detail -> Back to Search -> tap My Page | The original `task7-fix3-manual-tablet` detail/Back success claim is withdrawn: retained fix3/fix4 tablet XML did not prove event detail plus Search/list restoration. The accepted tablet detail/Back proof is superseded by the exact-SHA final evidence under `task7-final`, where a clean-installed app starts from root, opens Search/list, taps a visible event, reaches Event detail, uses system/app Back, and returns to the in-app Search/list rather than launcher/home. |
