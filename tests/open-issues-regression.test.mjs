...
async function assertHomeDesktopResaleMenu(browser, baseUrl) {
  ...
  try {
    ...
    const resaleMenu = page.getByRole("link", { name: "CLEAN 티켓 공식 양도", exact: true });
...
async function assertHomeMobileIssueFixes(browser, baseUrl) {
  ...
  try {
    ...
    const resaleLink = resaleSection.getByRole("link", { name: "공식 풀 보기" });
...
async function assertMypageOfficialResaleAction(browser, baseUrl) {
  ...
  try {
    ...
    const resaleLinks = main.getByRole("link", { name: /CLEAN 티켓 양도/ });
