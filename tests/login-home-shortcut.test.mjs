...
test("login and signup pages expose the home shortcut above the login card", async (t) => {
  ...
  for (const pathName of ["/login", "/signup"]) {
    ...
    try {
      ...
      const heroHeading = page.getByRole("heading", { name: /Tig 티켓 예매와/ });
      ...
      assert.equal(headingText.replace(/\s+/g, " ").trim(), "Tig 티켓 예매와 CLEAN 티켓 공식 양도를 한 계정에서 이용해보세요.");
      assert.match(headingHtml, /Tig 티켓 예매와 CLEAN 티켓 공식 양도를\s*<br/);
