...
## 13. 다크모드 후속 QA 수정 검증 (Sonnet 5, 2026-07-05)

> 검증 대상: 사용자가 실제 화면에서 발견한 5건의 다크모드 QA 이슈(기획전 카드 저대비, 히어로 이미지 워시아웃, 배경색 톤, 재판매/캘린더 링크 시각 약함, 헤더 토글 위치)에 대한 codex 구현. 커밋 `fe0d716`~`cf29304`(4개), 기준점은 `8135e5d`. 검증: Sonnet 5 1차 검증(자동 검사 + 소스 대조) → Fable advisor 재검증.
...
| `npm run lint` | 통과 (경고 0) |
| `npm run typecheck` | 통과 |
| `npm test` (전체 **100개** 테스트, build 포함) | **100 pass / 0 fail** |

### 13-2. 요청 이슈별 이행 확인

| 이슈 | 상태 | 확인 내용 |
|---|---|---|
| ① 기획전 3번(cream) 카드 저대비 | **완료, 근본 원인 정확히 수정** | `home-sections.tsx`의 `cream` 톤이 `text-ink`(테마 반전 토큰)에서 신규 `text-on-accent-2`(고정 `#1a1a1d`, `.dark`에서 재정의 안 됨)로 교체. `--accent-2`는 라이트/다크 모두 밝은 노랑을 유지하므로 대응하는 전경색이 고정 어두운 색이어야 한다는 §9-2 설계 원칙과 일치. |
| ② 히어로 이미지 다크모드 워시아웃 | **완료, 근본 원인 정확히 수정** | `home-cards.tsx`의 사진 위 그라데이션 스크림이 `from-ink/…`(반전 토큰)에서 신규 `from-scrim/…`(고정 `#000000`, `.dark`에서 재정의 안 됨)로 교체, 짝이 되는 전경색도 `text-on-ink`→고정 `text-white`로 변경. 사진 위 스크림은 테마와 무관하게 항상 어두워야 한다는 원칙에 맞게 신규 시맨틱 토큰(`--scrim`/`--on-scrim`)을 도입한 것으로, 지엽적 패치가 아니라 올바른 일반화. |
| ③ 다크 배경 톤 조정 | **완료, 요청 방향과 일치** | `--bg: #121214`→`#1b1b1e`, `--bg-2: #1c1c1f`→`#202024`, `--bg-4: #2b2b30`→`#27272c`(`--bg-3`는 `#232327` 유지)로 전반적으로 밝게 조정되었고 `bg < bg-2 < bg-3 < bg-4` 밝기 순서도 그대로 유지됨. 순검정에서 진회색 계열로 이동한 것을 확인. |
| ④ 재판매/캘린더 링크 시각 강화 | **완료** | `site-header.tsx`에 `RefreshCcw`/`CalendarDays` 아이콘 + 배지 스타일(`border`, `bg-card`, `shadow-ticket-1`, hover 시 `bg-ticketground`)을 적용해 순수 텍스트 링크에서 pill 버튼으로 전환. 신규 테스트 `tests/header-secondary-link-order.test.mjs`가 소스 문자열이 아니라 **실제 렌더링된 계산 스타일**(`background-color`가 투명이 아님, `border-radius ≥ 8px`, `display: flex`)을 검증 — codex가 언급한 "Gate Reviewer 지적에 따른 검증 방식 교체"가 실제로 반영됨. |
| ⑤ 헤더 ThemeToggle 위치 | **완료** | `site-header.tsx`에서 `ThemeToggle`이 `utilityLinksBeforeAuth`(고객센터) 매핑보다 앞으로 이동 확인 — 요청한 "고객센터 앞쪽" 배치와 정확히 일치. |
| /resale 페이지 UI 개선 | **완료, 범위 상회** | `resale-intro.tsx`/`resale-find-panel.tsx`/`resale-sell-panel.tsx`/`resale-audit-aside.tsx` 전반에 걸쳐 레이아웃 재구성(정책 요약을 히어로 카드 안으로 통합, 상태 표시 카드 분리 등)과 신규 `on-ink`/`on-accent-2` 토큰 적용 확인. |

### 13-3. 발견 사항

**[정보] 동일 패턴(반전 토큰을 스크림 용도로 오용)의 잔여 인스턴스 1건 — 이번 라운드 범위 밖**
`src/components/mobile-nav.tsx:35`의 `Dialog.Backdrop`이 여전히 `bg-ink/45`를 사용한다. 이는 ①·②와 동일한 클래스의 버그다: 모바일 드로어를 열 때 배경을 어둡게 하는 딤 처리용 오버레이인데, `--ink`가 다크모드에서 밝은 색으로 반전되므로 **다크모드에서 드로어를 열면 배경이 어두워지는 대신 반투명 흰색으로 밝아질 가능성**이 있다. 이번 요청 범위(사용자가 지목한 5건)에는 포함되지 않았던 항목이라 이번 라운드에서 손대지 않은 것은 합리적이나, 같은 원인의 버그이므로 후속 조치 시 `bg-ink/45` → `bg-scrim/45`로 교체를 권고한다.
