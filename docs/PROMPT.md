...
7. public 사용자 페이지에는 관리자 화면과 링크를 포함하지 않는다. 관리자 기능은 50084 포트 backend API로 분리한다.

[파일 구조]
assets/tokens.css, assets/app.css, tweaks-panel.jsx 와
public 사용자 화면. app.css가 tokens.css와 Pretendard CDN을 @import.
...
P2: resale → transfer → watchlist → inquiry → (confirm·mypage에 QR 상태 분리 반영)
...
- 관리자 기능은 public 화면에 포함하지 않고 50084 backend API로 분리한다.
...
- **6턴**: "resale·transfer·watchlist·inquiry."
